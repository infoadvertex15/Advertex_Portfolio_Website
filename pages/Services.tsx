...
export const Services: React.FC = () => {
  ...
  if (activeServiceId) {
    ...
    if (!service) return <div className="pt-32 text-center text-brand-primaryText">Service not found</div>;
      ...
      <div className="pt-20 min-h-screen">
        <div className="relative py-24 bg-brand-dark/50">
           <div className="container mx-auto px-6 relative z-10">
              ...
              <h1 className="text-5xl md:text-6xl font-bold mb-6 text-brand-primaryText">{service.title}</h1>
              <p className="text-xl text-brand-secondaryText max-w-3xl">{service.fullDescription}</p>
           </div>
        </div>

        <div className="container mx-auto px-6 py-20 grid lg:grid-cols-2 gap-16">
          <div>
            <h3 className="text-2xl font-bold mb-6 text-brand-primaryText">Key Deliverables</h3>
            <div className="grid gap-4">
              {service.features.map((feature, i) => (
                <div key={i} className="flex items-center gap-4 p-4 rounded-xl bg-black/5 dark:bg-white/5 border border-black/5 dark:border-white/5 text-brand-primaryText">
            ...
            <div className="mt-12 p-8 rounded-2xl bg-gradient-to-br from-brand-teal/10 to-transparent border border-brand-teal/20">
              <h4 className="text-xl font-bold text-brand-primaryText mb-4">Ready to get started?</h4>
              <p className="text-brand-secondaryText mb-6">Book a consultation specific to {service.title}.</p>
              <Button onClick={() => navigate('/contact')}>Get In Touch</Button>
            </div>
          </div>

          <div>
             <h3 className="text-2xl font-bold mb-6 text-brand-primaryText">Our Process</h3>
             <div className="space-y-8">
               {service.process.map((step, i) => (
                 <div key={i} className="relative pl-10">
                    ...
                    <h4 className="text-xl font-bold mb-2 text-brand-primaryText">{step}</h4>
                    <p className="text-brand-secondaryText text-sm">We execute {step.toLowerCase()} with precision to ensure maximum impact for your campaign.</p>
                 </div>
               ))}
             </div>
          </div>
    ...
    <div className="pt-32 pb-20 min-h-screen">
      <div className="container mx-auto px-6 text-center mb-20">
        <SectionHeading 
          ...
          center 
        />
        <p className="text-xl text-brand-secondaryText max-w-2xl mx-auto">
        ...
        {SERVICES.map((service) => (
          <Card key={service.id} className="h-full flex flex-col justify-between group cursor-pointer" hoverEffect>
             <div onClick={() => setSearchParams({ id: service.id })}>
                <div className="w-16 h-16 rounded-2xl bg-black/5 dark:bg-white/5 flex items-center justify-center text-brand-primaryText mb-8 group-hover:bg-brand-teal group-hover:text-brand-black transition-colors duration-300">
                  <service.icon size={32} />
                </div>
                <h3 className="text-2xl font-bold mb-4 text-brand-primaryText">{service.title}</h3>
                <p className="text-brand-secondaryText leading-relaxed mb-8">
